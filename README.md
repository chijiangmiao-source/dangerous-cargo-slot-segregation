...
返回**首个冲突**的完整证据；无冲突则返回合规及实际比较对数。另提供箱位推荐
接口：整舱裁决完成后，为待装箱从期望坐标出发按曼哈顿距离逐层搜索最近的
安全箱位。基于 Python 3.13 + FastAPI + Pydantic v2，规则与接口全量 pytest
覆盖，提供 Docker Compose 与名为 `verify` 的一次性验收服务。
...
│   ├── __init__.py
│   ├── main.py              # FastAPI 入口（/healthz、adjudicate、recommend-slot）
│   ├── models.py            # 请求/响应模型 + 整份输入校验（422）
│   └── rules.py             # 规则表、曼哈顿距离、裁决/箱位搜索（纯函数）
├── tests/                   # pytest 单元/接口测试
│   ├── test_rules.py
│   ├── test_api.py
│   └── test_recommend_slot.py
├── scripts/
...
### `POST /api/v1/recommend-slot`

整舱裁决完成后，为**一个待装箱**寻找离期望坐标最近的安全箱位。合法请求
**不改变任何已有箱位**，只判断待装箱与各现存箱的隔离关系；现存箱之间的
冲突不影响推荐（沿用裁决接口的整份校验，但不要求现存布局本身合规）。

#### 请求体

| 字段 | 类型 | 约束 |
|---|---|---|
| `max_row` / `max_col` / `max_tier` | int | 舱段尺寸，**必须 ≥ 1**（同裁决接口） |
| `containers` | array | 现存箱位，可为空；越界、重复、未知类别整份 422（同裁决接口） |
| `category` | string | **待装箱类别**，仅 `"A"` `"B"` `"C"` `"D"`（大小写敏感） |
| `expected.row/col/tier` | int | **期望坐标**，`1 ≤ 坐标 ≤ max_*`，必须在舱内 |

```json
{
  "max_row": 5, "max_col": 5, "max_tier": 5,
  "containers": [{"row": 3, "col": 3, "tier": 3, "category": "A"}],
  "category": "B",
  "expected": {"row": 3, "col": 3, "tier": 3}
}
```

#### 搜索规则

* 从期望点出发按**曼哈顿距离逐层展开**（半径 0、1、2、…），只生成落在舱内
  的壳层坐标，已占用位置直接排除，**不为大尺寸稀疏舱段物化完整网格**；
* 候选必须对**全部**现存箱满足 `距离 ≥ 要求距离`（沿用同一张固定规则表，
  临界相等即可）；要求距离最大为 3，安全性按类别查半径 ≤ 2 的常数邻域，
  与现存箱数量无关；
* 同一壳层存在并列安全点时，按 `(排, 列, 层)` **字典序取首项**，结果与
  现存箱的输入顺序无关；
* 全舱没有任何安全位置时返回无解状态。

#### 响应（同一 `status` 字段区分两种结果）

找到 → `200`，`search_distance` 即中选点到期望点的曼哈顿距离：
...
期望坐标越界、待装类别未知（含小写）、类型错误（尺寸/坐标/类别为字符串、
数字或浮点）、现存箱越界或重复占位、缺字段或多字段、空请求体，一律整份
`422`，不返回任何推荐字段。多个问题并存时仍只回一次 422。
...
1. 等待 `api` 健康检查通过后启动；
2. 执行 160+ 项固定检查：三类特殊规则的**临界距离稳定判合规**、
   低一格组合产生**唯一冲突**、冲突证据由脚本侧**独立按曼哈顿距离复算**、
   多冲突首项排序及输入乱序不变性、比较对数，推荐箱位的并列取字典序首位
   （含 24 种排列与暴力枚举复核）、小舱无解不含坐标、非法输入整份 422；
...
覆盖（105 项）：规则矩阵完整性与对称性、曼哈顿三轴定义、非欧氏判定、
空位不折叠、临界相等合规、低一格冲突、首冲突双层排序、类别不参与排序、
20 次乱序可复现性、比较对数计数、三个健康地址与探针脚本退出码，全部
422 边界（尺寸非正、上下界越界、重复、未知类别、缺字段、多字段、类型错误、
空体），以及推荐箱位的逐层搜索与字典序并列取首、多排列输入不变性、
全箱对隔离约束、期望点占用与舱段上界、无解状态不含坐标与推荐接口的
整份 422 校验。
...
* 校验在 Pydantic 模型层（含跨字段的上界、期望坐标与重复检测，聚合后一次
  拒绝），进入业务逻辑的数据一定合法，故裁决/搜索路径无需再处理非法输入。
* 箱位搜索只枚举以期望点为中心的曼哈顿壳层（各轴偏移与舱段边界求交、并按
  其余两轴容量收紧下界），安全性用半径 ≤ 2 的类别邻域常数查表，不物化完整
  网格；最坏时间为 O(舱格数)，与现存箱数量解耦。
